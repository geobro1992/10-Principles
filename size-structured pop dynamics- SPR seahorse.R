  ...
  #Life history parameters      	
   recruitsize=.5
    Amax=5 #max age
 	...
 	amat=1 #age at maturation - this varies depending on the matration function chosen
	Linf=16 #max size
	 fishingsize = .75*Linf #size-selective fishery threshold size 
        
       k=2.9   # Growth function param

	 	 
	  kappa=0.3  #natural mortality coef  
...
#natural mortality
   mu[a]= kappa + eps/L[a]  #natural mortality function of Length (Lorenzen curve)
...
P[t]= sum(E[,t])  #assuming fertilization is 100%
 ...
 par(mfrow=c(3,2))
  ...
  R=alpha*P_/(1+beta*P_)
   ...
   plot(sums[1:(Tmax-1)], N[1,2:Tmax], main="Stock Recruitment Curve", xlab="Spawners", ylab="1st time spawners", type="l", lwd=1.5)
