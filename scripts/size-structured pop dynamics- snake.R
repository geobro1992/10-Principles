...
recruitsize=10
Amax=15 #max age
amat=4 #age at maturation - this varies depending on the matration function chosen

Linf=60 #max size
k=0.3   # Growth function param
fishingsize = .75*Linf #size-selective fishery threshold size 


M=0.22   #natural mortality coef  

c=0.38   #egg slope 
d=-13.6   #egg intercept

N0 = 100000*Linf  # intitial number of eggs     
...
N[1:Amax, 1]=Npop #initial population size
...
par(bg=NA)
plot(1:Tmax, sums, type="l", lwd = 10, col = "#1b9e77", axes = F, xlab="", ylab="")

dev.copy(png,'snake.png')
dev.off()
...
for(t in 1:(Tmax-1)) {
  ...
  if (Tfishing < t & Trecovery > t) {
    N[1,t+1]= (alpha*P[t]/(1+beta*P[t])) * 0.5 } else {
      N[1,t+1]= alpha*P[t]/(1+beta*P[t]) #this is the N_0 class that is born and enters in the next time step.. 
    }
